...
import { Bike, Navigation, Wallet, LogOut, Bell } from 'lucide-react';
import { useAuthStore } from '@/store/authStore';

const NAV = [
  { href: '/delivery/home', label: 'Home', icon: Bike },
  { href: '/delivery/active-order', label: 'Active', icon: Navigation },
...
export default function DeliveryLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950 font-sans selection:bg-blue-500/30 text-gray-900 dark:text-gray-100">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 p-4 shrink-0">
        <div className="flex items-center gap-3 px-2 py-4 mb-6">
          ...
          <div className="flex flex-col">
            <span className="font-black text-xl tracking-tight leading-none">Rider</span>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Partner App</span>
          </div>
        </div>
        ...
        <nav className="flex-1 flex flex-col gap-2">
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-bold transition-all ${isActive ? 'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
      ...
      <main className="flex-1 flex flex-col relative max-w-full overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between p-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 z-10 sticky top-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-lg tracking-tight">Rider App</span>
          </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-around p-2 pb-safe z-50">
          {NAV.map(item => {
            const isActive = pathname === item.href;
            return (
              <Link key={item.href} href={item.href} className={`flex flex-col items-center justify-center w-16 h-14 rounded-2xl transition-all ${isActive ? 'text-blue-500' : 'text-gray-400'}`}>
